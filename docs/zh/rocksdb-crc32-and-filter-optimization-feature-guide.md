...
本文主要介绍RocksDB数据库CRC32和Filter优化特性的优化原理和安装使用方法。
...
本特性通过硬件加速CRC32减少校验开销和智能布隆过滤器减少无效IO，提升RocksDB在鲲鹏服务器中的整体性能表现与系统稳定性。
...
RocksDB CRC32及Filter优化特性针对RocksDB 6.1.2版本进行开发，以patch文件形式提供。安装和使用该特性需先在RocksDB源码中应用该patch文件，再编译RocksDB。
...
6. （可选）通过YCSB工具测试可以得到使能本特性前后的性能提升效果，详细测试步骤请参见《[YCSB测试指导](https://www.hikunpeng.com/document/detail/zh/kunpengdbs/testguide/tstg/kunpengycsbformong_11_0001.html)》。RocksDB CRC32及Filter优化特性可以使16U规格下两个用例workloada和workloadc的性能平均提升5%，优化前后对比效果如[**图 1** 优化特性使能前后性能对比](#优化特性使能前后性能对比)所示。

    **图 1** 优化特性使能前后性能对比<a name="fig1620685015314"></a><a id="优化特性使能前后性能对比"></a><br>
